...
@Injectable()
export class CompleteLabOrderUsecase extends Usecase<TResult, TParams> {
  constructor(
  ...
  async execute(em: EntityManager, params: TParams): Promise<TResult> {
    ...
    if (existing.status !== LabOrderStatusEnum.PROCESSING) {
      throw new BadRequestException(`Cannot complete lab order in status '${existing.status}'.`);
    }

    const labOrder = await this.labOrderService.updateLabOrder(
      { id: params.id },
      { status: LabOrderStatusEnum.COMPLETED, completedAt: new Date() },
      em,
    );
    ...
    await this.eventService.log(
      {
        ...
        event: EventType.LAB_ORDER_COMPLETED,
