...
@Injectable()
export class CancelLabOrderUsecase extends Usecase<TResult, TParams> {
  constructor(
  ...
  async execute(em: EntityManager, params: TParams): Promise<TResult> {
    ...
    const cancellableStatuses: LabOrderStatusEnum[] = [
    ...
    if (!cancellableStatuses.includes(existing.status)) {
      throw new BadRequestException(
        `Cannot cancel lab order in status '${existing.status}'. Only pending or collected orders can be cancelled.`,
      );
    }

    const labOrder = await this.labOrderService.updateLabOrder(
      { id: params.id },
      { status: LabOrderStatusEnum.CANCELLED },
      em,
    );
    ...
    await this.eventService.log(
      {
        ...
        event: EventType.LAB_ORDER_CANCELLED,
