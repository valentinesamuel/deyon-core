...
const CANCELLABLE_STATUSES = [AppointmentStatusEnum.SCHEDULED, AppointmentStatusEnum.CONFIRMED];

type TParams = { id: string };
type TResult = { appointment: Appointment };

@Injectable()
export class CancelAppointmentUsecase extends Usecase<TResult, TParams> {
  constructor(
  ...
  async execute(em: EntityManager, params: TParams): Promise<TResult> {
    ...
    if (!CANCELLABLE_STATUSES.includes(existing.status)) {
      throw new BadRequestException('Appointment cannot be cancelled in its current state');
    }
